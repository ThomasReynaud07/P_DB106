...
-- docker cp ScriptBackup.sql db:/var/lib/mysql-files/
...
-- Pour faire une Backup complète executé ça dans un cmd windows
-- docker exec -i db mysqldump -u root -proot db_thanos_pizzeria > "C:\Users\pk84vtk\Desktop\backup.sql" <-- Ceci est le chemin dans lequel sera creer le backup changé le selon votre souhait
-- docker cp C:\Users\pk84vtk\Desktop\backup.sql db:/var/lib/mysql-files/

-- Rajout d'un livreur
INSERT INTO t_livreur (livreur_id, nom, actif) VALUES (7, 'Léo', 1);

-- Avant le prenom du client 4 était Amina
UPDATE t_livreur
SET nom = 'Jean'
WHERE livreur_id = 4;

-- Backup différentielle
-- docker exec -i db mysqldump -u root -proot --where="date_modification >= '2025-10-31'" db_thanos_pizzeria t_livreur > "C:\Users\pk84vtk\Desktop\backupDiff.sql" <-- Ceci est le chemin dans lequel sera creer le backup changé le selon votre souhait
-- docker cp C:\Users\pk84vtk\Desktop\backupDiff.sql db:/var/lib/mysql-files/

-- Restaurer la db
/*
1) Sert a restaurer la backup complète | IL FAUT EXECUTER CETTE COMMANDE DANS LA CONSOLE MYSQL DE DOCKER AU MOMENT DE SE CONNECTER DONC A L'ENDROIT OU C'EST ECRIT sh-4.4#
 mysql -u root -proot db_thanos_pizzeria < "/var/lib/mysql-files/backup.sql"
2) Sert a restaurer la backup differentielle | IL FAUT EXECUTER CETTE COMMANDE DANS LA CONSOLE MYSQL DE DOCKER AU MOMENT DE SE CONNECTER DONC A L'ENDROIT OU C'EST ECRIT sh-4.4#
 mysql -u root -proot db_thanos_pizzeria < "/var/lib/mysql-files/backupDiff.sql"    
*/
